...
public interface RamRepository extends JpaRepository<RAM, Integer> {
    ...
    @Query("SELECT r FROM RAM r WHERE r.size <= :maxSize AND (r.size*1024) >= :reqSize AND r.sticks <= :sticks AND r.ram_type = :type AND r.rank IS NOT NULL ORDER BY r.rank DESC LIMIT 1")
